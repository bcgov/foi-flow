
export const WEB_BASE_URL = (window._env_ && window._env_.REACT_APP_WEB_BASE_URL) || process.env.REACT_APP_WEB_BASE_URL;

export const FOI_BASE_API_URL = `${(window._env_ && window._env_.REACT_APP_FOI_BASE_API_URL) || process.env.REACT_APP_FOI_BASE_API_URL}`;

export const AXIS_API_URL = `${(window._env_ && window._env_.REACT_APP_AXIS_API_URL) || process.env.REACT_APP_AXIS_API_URL}`;

export const DOC_REVIEWER_BASE_API_URL = `${(window._env_ && window._env_.REACT_APP_DOCREVIEWER_BASE_API_URL) || process.env.REACT_APP_DOCREVIEWER_BASE_API_URL}`;

export const FOI_TEMPLATE_API_URL = `${(window._env_ && window._env_.REACT_APP_FOI_TEMPLATE_API_URL) || process.env.REACT_APP_FOI_TEMPLATE_API_URL}`;

export const DOC_EDITOR_API_URL = `${(window._env_ && window._env_.REACT_APP_DOC_EDITOR_API_URL) || process.env.REACT_APP_DOC_EDITOR_API_URL}`;