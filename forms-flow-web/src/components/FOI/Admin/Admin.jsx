...
import { refreshRedisCacheForAdmin, refreshRedisCacheForTemplate } from "../../../apiManager/services/FOI/foiMasterDataServices";
...
const Admin = ({ userDetail }) => {
  ...
  const refreshRedisCache = () => {
    ...
    // Refresh both Admin and Template Redis Caches
    Promise.all([
      new Promise((resolve) => {
        dispatch(refreshRedisCacheForAdmin((err, res) => {
          if (!err && res) {
            toast.success(res.message ? res.message : "Admin Cache refreshed successfully.", {
              position: "top-right",
              autoClose: 3000,
            ...
            toast.error("Temporarily unable to refresh Admin cache. Please try again in a few minutes.", {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: true,
              closeOnClick: true,
      ...
      new Promise((resolve) => {
        dispatch(refreshRedisCacheForTemplate((err, res) => {
          if (!err && res) {
            toast.success(res.message ? res.message : "Template Cache refreshed successfully.", {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: true,
              closeOnClick: true,
            ...
            toast.error("Temporarily unable to refresh Template cache. Please try again in a few minutes.", {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: true,
              closeOnClick: true,
