...
const IAOOpenInfoPublishing = ({
  ...
  const [oiPublicationData, setOiPublicationData] =
    useState<OITransactionObject | null>(foiOITransactionData);
  ...
  return (
    <div className="oi-section">
      ...
      <button
        ...
        onClick={handleSave}
        disabled={
          (oiPublicationData?.oiexemption_id === null &&
            oiPublicationData?.oipublicationstatus_id === 1) ||
          requestDetails?.oiPublication === null
        }
...
3. trigger update route
4. finalzie FE ux for iao side of things
5. trigger post route in FE to post foioi txn data (done when save button clicked and when request is closed)
*/
