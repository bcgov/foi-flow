...
namespace FOIMOD.CFD.DocMigration.Utils.UnitTests
{
    ...
    public class S3UploadTest
    {
        ...
        {
            PDFDocToMerge[] pDFDocToMerges = new PDFDocToMerge[2];
            pDFDocToMerges[0] = new PDFDocToMerge() { PageFilePath = Path.Combine(getSourceFolder(), "DOCX1.pdf"), PageSequenceNumber = 2 };
            pDFDocToMerges[1] = new PDFDocToMerge() { PageFilePath = Path.Combine(getSourceFolder(), "cat1.pdf"), PageSequenceNumber = 1 };
