...
class FOIRequestNotification(object):
    ...
    def savenotification(self, notificationschema):
        ...
        try:
            ...
            conn = getconnection()
            cursor = conn.cursor()
            cursor.execute('INSERT INTO public."FOIRequestNotifications" (notification, notificationtypeid, requestid, "version", idnumber, axisnumber, foirequestid, createdby, created_at) \
                                VALUES(%s::json,%s::integer, %s::integer, %s::integer,%s,%s,%s::integer,%s,%s) RETURNING notificationid', 
                                (json.dumps(notificationschema.notification), int(notificationschema.notificationtypeid), int(notificationschema.requestid), int(notificationschema.version),
