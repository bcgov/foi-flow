...
class pdpublishdateevent(duecalculator):
    ...
    def createdueevent(self):
        try: 
            ...
            _upcomingdues = FOIMinistryRequest.getupcomingpublicationduerecords()
            notificationtype = NotificationType().getnotificationtypeid(self.__notificationtype())
            for entry in _upcomingdues:
                _duedate = self.formatduedate(entry['publicationdate'])
                message = None
                if  _duedate == _today:                
                    message = self.__todayduemessage()     
                elif  self.getpreviousbusinessday(entry['publicationdate'],ca_holidays) == _today or self.getbusinessdaysbetween(entry['publicationdate'],_today) == 5:
            ...
            return DefaultMethodResult(True,'Publication reminder notifications created',_today)
        except BusinessException as exception:            
            current_app.logger.error("%s,%s" % ('Publication reminder Notification Error', exception.message))
            return DefaultMethodResult(False,'Publication reminder notifications failed',_today)
    ...
    def __upcomingduemessage(self, duedate):
        return 'Publication Date due on ' + parse(str(duedate)).strftime("%Y %b %d").upper()  
   
    def __todayduemessage(self):
        return 'Publication Due Date is Today'
    
    def __notificationtype(self):
        return "Publication Due Reminder"
