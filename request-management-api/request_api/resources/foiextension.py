        ...
        try:                     
            ...
            if (AuthHelper.isministrymember() == False):           
                ...
                if result.success == True:
                    asyncio.create_task(eventservice().posteventforextension(ministryrequestid, result.identifier, AuthHelper.getuserid(), AuthHelper.getusername(), "add"))
    ...
    def post(requestid, ministryrequestid, extensionid):
        try:         
            if (AuthHelper.isministrymember() == False):           
                ...
                if result.success == True:
                    asyncio.create_task(eventservice().posteventforextension(ministryrequestid, extensionid, AuthHelper.getuserid(), AuthHelper.getusername(), "delete"))
                    return {'status': result.success, 'message':result.message,'id':result.identifier} , 200
        except KeyError as err:
            return {'status': False, 'message':err.messages}, 400        
        except BusinessException as exception:            
