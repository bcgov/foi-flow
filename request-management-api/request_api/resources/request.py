    ...
    def post(requestid=None, actiontype=None):
        try :                        
            ...
            if int(requestid) and str(requestid) != "-1" :
                ...
                if status not in [StateName.intakeinprogress.value, StateName.closed.value, StateName.redirect.value, StateName.peerreview.value, StateName.section5pending.value, StateName.appfeeowing.value, StateName.onholdother.value, StateName.unopened.value]:
