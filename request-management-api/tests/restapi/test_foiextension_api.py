...
def test_foiministryextension_list(app, client):
    ...
    foiministryextnresponse = client.post('/api/foiextension/ministryrequest/'+str(foijsondata["ministryRequests"][0]["id"]),data=json.dumps(extensionschema), headers=factory_intake_auth_header(app, client), content_type='application/json')
    getextensionssresponse = client.get('/api/foiextension/ministryrequest/'+str(foijsondata["id"]), headers=factory_ministry_auth_header(app, client), content_type='application/json')
    assert rawresponse.status_code == 200 and foiresponse.status_code == 200 and wfupdateresponse.status_code == 200 and foiministryresponse.status_code == 200 and foiministryresponse2.status_code == 200 and foiministryextnresponse.status_code == 200 and getextensionssresponse.status_code == 200
