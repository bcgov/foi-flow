...
def test_foirawwatcher(app, client):
    ...
    disablewatcherresponse = client.put('/api/foiwatcher/rawrequest/'+str(jsondata["id"]), headers=factory_auth_header(app, client), content_type='application/json')
...
def test_foiministrywatcher(app, client):
    ...
    createwatcherresponse = client.post('/api/foiwatcher/ministryrequest', data=json.dumps(watcherjson), headers=factory_auth_header(app, client), content_type='application/json')
    getwatcherresponse = client.get('/api/foiwatcher/ministryrequest/'+str(foijsondata["id"]), headers=factory_auth_header(app, client), content_type='application/json')
    disablewatcherresponse = client.put('/api/foiwatcher/rawrequest/'+str(jsondata["id"]), headers=factory_auth_header(app, client), content_type='application/json')
