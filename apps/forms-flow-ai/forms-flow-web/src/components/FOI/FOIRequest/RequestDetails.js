...
const RequestDetails = React.memo(({requestDetails, handleRequestDetailsValue, handleRequestDetailsInitialValue, createSaveRequestObject}) => {
    ...
    const receivedModes = receivedMode.map((item) => {    
      return ( <MenuItem key={item.name} value={item.name} disabled={item.name.toLowerCase().includes("select") || (item.name === FOI_COMPONENT_CONSTANTS.ONLINE_FORM && requestDetails.receivedMode && requestDetails.receivedMode.toLowerCase() !== FOI_COMPONENT_CONSTANTS.ONLINE_FORM.toLowerCase())}>{item.name}</MenuItem> )
    });
            ...
            <CardContent>            
                <div className="row foi-details-row">
                    ...
                    <TextField
                            ...
                            disabled={requestDetails.receivedMode && requestDetails.receivedMode.toLowerCase() === FOI_COMPONENT_CONSTANTS.ONLINE_FORM.toLowerCase() || requestDetails.currentState && requestDetails.currentState.toLowerCase() === FOI_COMPONENT_CONSTANTS.UNOPENED }                         
                        >            
                        {receivedModes}
                        </TextField> 
                    <TextField
