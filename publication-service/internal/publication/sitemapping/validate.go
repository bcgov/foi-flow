...
func NewValidator(allowedSources []string) (*Validator, error) {
	schema, err := events.SchemaFor(events.TypePublicationSitemappingRequested, events.SchemaVersionV1)
	if err != nil {
		return nil, fmt.Errorf("publication.sitemapping: validator schema: %w", err)
	}
...
func (v *Validator) Validate(env *events.Envelope) error {
	if env.EventType != events.TypePublicationSitemappingRequested {
